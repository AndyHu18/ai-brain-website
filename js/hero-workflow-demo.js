...
class HeroWorkflowDemo {
  ...
  init() {
    ...
    const wrapper = this.container.closest(".hero-workflow-container");
    if (!wrapper) return;

    const revealAndStart = () => {
      wrapper.classList.add("revealed");
      // 等 display:block 生效後渲染連接線
      setTimeout(() => {
    ...
    // 檢查 wrapper 的位置是否已在視窗下方（需要滾動才看到）
    const rect = wrapper.getBoundingClientRect();
    if (rect.top > window.innerHeight) {
      // 不在視窗內：用 scroll listener 等使用者滾動到附近時觸發
      const onScroll = () => {
      ...
      // 已在視窗內或接近：延遲 1.5 秒後直接展開
      setTimeout(revealAndStart, 1500);
    }
  }

  renderNodes() {
    this.nodesContainer.innerHTML = this.nodes
      .map(
        (node, index) => `
  ...
  renderConnections() {
    ...
    for (let i = 0; i < this.nodes.length - 1; i++) {
      ...
      if (fromNode && toNode) {
        ...
        console.log(
          `📍[HeroWorkflow] 連接線 ${i}: (${startX.toFixed(0)}, ${startY.toFixed(0)}) -> (${endX.toFixed(0)}, ${endY.toFixed(0)})`,
        );
      }
    }
